...
> **Version compatibility (important)**: code generated by `protoc_plugin` 25.x
> requires the Dart `protobuf` package **^6.0.0**. If the app pins a lower
> version (e.g. ^3), the generated code fails with errors like
> `The method 'aI' isn't defined for the type 'BuilderInfo'` — bump
> `protobuf` to `^6.0.0` (see §3.2).
...
```yaml
dependencies:
  protobuf: ^6.0.0        # must match protoc_plugin 25.x (see §0.2)
  fixnum: ^1.0.0
