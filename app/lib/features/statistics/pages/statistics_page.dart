...
class StatisticsPage extends GetView<StatisticsController> {
          ...
          child: Column(
            children: [
              ...
              Expanded(
                child: TabBarView(
                  children: [
                    _buildStatsPage(TimePeriod.week),
                    _buildStatsPage(TimePeriod.month),
                    _buildStatsPage(TimePeriod.all),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildStatsPage(TimePeriod period) {
    return Obx(() {
      ...
      final DateTime currentCutoff = switch (period) {
      ...
      final Duration periodDuration = switch (period) {
      ...
      final currentCompleted = controller.allTasks
          .where((t) => DateTime.fromMillisecondsSinceEpoch(t.completedAt.toInt()).isAfter(currentCutoff))
          .toList();
      final prevCompleted = controller.allTasks
          .where(
            (t) =>
                DateTime.fromMillisecondsSinceEpoch(t.completedAt.toInt()).isAfter(prevCutoff) &&
                DateTime.fromMillisecondsSinceEpoch(t.completedAt.toInt()).isBefore(currentCutoff),
          )
          .toList();
      ...
      // Lifetime EXP = sum of the requirements for each finished level
      // plus the current level's internal progress.
      final _char = UserService.to.character.value;
      var totalXp = _char?.currentExp.toInt() ?? 0;
      final _lvl = _char?.level ?? 1;
      for (var l = 1; l < _lvl; l++) {
        totalXp += GameConstants.expForLevel(l);
      }
          ...
          Row(
            children: [
              _kpiCard(
                '$totalCompleted',
                LanKey.tasksDONE.tr,
                period != TimePeriod.all && rateChange != 0
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: TabBar(
          ...
          tabs: [
            Tab(text: LanKey.week.tr),
            Tab(text: LanKey.month.tr),
