...
class GameLogic {
  ...
  static Character takeDamage(Character c, int amount) {
    ...
    return c.rebuild((x) => x
      ...
      ..deathRecoveryUntil = dead
          ? Int64(DateTime.now().add(const Duration(minutes: GameConstants.deathRecoveryMinutes)).millisecondsSinceEpoch)
          : Int64.ZERO,
      );
  }

  /// Revives a dead character with the recovery HP.
  static Character revive(Character c) => c.rebuild((x) => x
    ..isDead = false
  ...
  static UserPrefs addCompletedTask(UserPrefs p) => p.rebuild((x) => x..totalTasksCompleted = p.totalTasksCompleted + 1);
}
