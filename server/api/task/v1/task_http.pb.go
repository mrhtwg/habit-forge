// Code generated by protoc-gen-go-http. DO NOT EDIT.
// versions:
// - protoc-gen-go-http v3.0.0
// - protoc             (unknown)
// source: api/task/v1/task.proto

package v1

import (
	context "context"
	http "github.com/go-kratos/kratos/v2/transport/http"
	"github.com/go-kratos/kratos/v2/transport/http/binding"
)

// This is a compile-time assertion to ensure that this generated file
// is compatible with the kratos package it is being compiled against.
var _ = new(context.Context)

const _ = http.SupportPackageIsVersion1

const OperationTaskServiceCompleteTask = "/api.task.v1.TaskService/CompleteTask"
const OperationTaskServiceCreateTask = "/api.task.v1.TaskService/CreateTask"
const OperationTaskServiceDeleteTask = "/api.task.v1.TaskService/DeleteTask"
const OperationTaskServiceGetTask = "/api.task.v1.TaskService/GetTask"
const OperationTaskServiceListTasks = "/api.task.v1.TaskService/ListTasks"
const OperationTaskServiceSkipTask = "/api.task.v1.TaskService/SkipTask"
const OperationTaskServiceUpdateTask = "/api.task.v1.TaskService/UpdateTask"

type TaskServiceHTTPServer interface {
	// CompleteTask CompleteTask marks a task completed and grants EXP/gold rewards.
	CompleteTask(context.Context, *CompleteTaskRequest) (*CompleteTaskReply, error)
	// CreateTask CreateTask creates a new task.
	CreateTask(context.Context, *CreateTaskRequest) (*CreateTaskReply, error)
	// DeleteTask DeleteTask removes a task.
	DeleteTask(context.Context, *DeleteTaskRequest) (*DeleteTaskReply, error)
	// GetTask GetTask returns one task by id.
	GetTask(context.Context, *GetTaskRequest) (*GetTaskReply, error)
	// ListTasks ListTasks lists tasks for the current user with optional filters.
	ListTasks(context.Context, *ListTasksRequest) (*ListTasksReply, error)
	// SkipTask SkipTask skips a task (todos are postponed to tomorrow).
	SkipTask(context.Context, *SkipTaskRequest) (*SkipTaskReply, error)
	// UpdateTask UpdateTask updates an existing task (title, difficulty, tags, skip, etc.).
	UpdateTask(context.Context, *UpdateTaskRequest) (*UpdateTaskReply, error)
}

func RegisterTaskServiceHTTPServer(s *http.Server, srv TaskServiceHTTPServer) {
	r := s.Route("/")
	r.Handle("GET", "/api/v1/tasks", _TaskService_ListTasks0_HTTP_Handler(srv))
	r.Handle("GET", "/api/v1/tasks/{id}", _TaskService_GetTask0_HTTP_Handler(srv))
	r.Handle("POST", "/api/v1/tasks", _TaskService_CreateTask0_HTTP_Handler(srv))
	r.Handle("PUT", "/api/v1/tasks/{id}", _TaskService_UpdateTask0_HTTP_Handler(srv))
	r.Handle("DELETE", "/api/v1/tasks/{id}", _TaskService_DeleteTask0_HTTP_Handler(srv))
	r.Handle("POST", "/api/v1/tasks/{id}/complete", _TaskService_CompleteTask0_HTTP_Handler(srv))
	r.Handle("POST", "/api/v1/tasks/{id}/skip", _TaskService_SkipTask0_HTTP_Handler(srv))
}

func _TaskService_ListTasks0_HTTP_Handler(srv TaskServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in ListTasksRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationTaskServiceListTasks)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.ListTasks(ctx, req.(*ListTasksRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*ListTasksReply)
		return ctx.Result(200, reply)
	}
}

func _TaskService_GetTask0_HTTP_Handler(srv TaskServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in GetTaskRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		if err := ctx.BindVars(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationTaskServiceGetTask)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.GetTask(ctx, req.(*GetTaskRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*GetTaskReply)
		return ctx.Result(200, reply)
	}
}

func _TaskService_CreateTask0_HTTP_Handler(srv TaskServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in CreateTaskRequest
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationTaskServiceCreateTask)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.CreateTask(ctx, req.(*CreateTaskRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*CreateTaskReply)
		return ctx.Result(200, reply)
	}
}

func _TaskService_UpdateTask0_HTTP_Handler(srv TaskServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in UpdateTaskRequest
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		if err := ctx.BindVars(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationTaskServiceUpdateTask)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.UpdateTask(ctx, req.(*UpdateTaskRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*UpdateTaskReply)
		return ctx.Result(200, reply)
	}
}

func _TaskService_DeleteTask0_HTTP_Handler(srv TaskServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in DeleteTaskRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		if err := ctx.BindVars(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationTaskServiceDeleteTask)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.DeleteTask(ctx, req.(*DeleteTaskRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*DeleteTaskReply)
		return ctx.Result(200, reply)
	}
}

func _TaskService_CompleteTask0_HTTP_Handler(srv TaskServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in CompleteTaskRequest
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		if err := ctx.BindVars(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationTaskServiceCompleteTask)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.CompleteTask(ctx, req.(*CompleteTaskRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*CompleteTaskReply)
		return ctx.Result(200, reply)
	}
}

func _TaskService_SkipTask0_HTTP_Handler(srv TaskServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in SkipTaskRequest
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		if err := ctx.BindVars(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationTaskServiceSkipTask)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.SkipTask(ctx, req.(*SkipTaskRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*SkipTaskReply)
		return ctx.Result(200, reply)
	}
}

type TaskServiceHTTPClient interface {
	// CompleteTask CompleteTask marks a task completed and grants EXP/gold rewards.
	CompleteTask(ctx context.Context, req *CompleteTaskRequest, opts ...http.CallOption) (rsp *CompleteTaskReply, err error)
	// CreateTask CreateTask creates a new task.
	CreateTask(ctx context.Context, req *CreateTaskRequest, opts ...http.CallOption) (rsp *CreateTaskReply, err error)
	// DeleteTask DeleteTask removes a task.
	DeleteTask(ctx context.Context, req *DeleteTaskRequest, opts ...http.CallOption) (rsp *DeleteTaskReply, err error)
	// GetTask GetTask returns one task by id.
	GetTask(ctx context.Context, req *GetTaskRequest, opts ...http.CallOption) (rsp *GetTaskReply, err error)
	// ListTasks ListTasks lists tasks for the current user with optional filters.
	ListTasks(ctx context.Context, req *ListTasksRequest, opts ...http.CallOption) (rsp *ListTasksReply, err error)
	// SkipTask SkipTask skips a task (todos are postponed to tomorrow).
	SkipTask(ctx context.Context, req *SkipTaskRequest, opts ...http.CallOption) (rsp *SkipTaskReply, err error)
	// UpdateTask UpdateTask updates an existing task (title, difficulty, tags, skip, etc.).
	UpdateTask(ctx context.Context, req *UpdateTaskRequest, opts ...http.CallOption) (rsp *UpdateTaskReply, err error)
}

type TaskServiceHTTPClientImpl struct {
	cc *http.Client
}

func NewTaskServiceHTTPClient(client *http.Client) TaskServiceHTTPClient {
	return &TaskServiceHTTPClientImpl{client}
}

// CompleteTask CompleteTask marks a task completed and grants EXP/gold rewards.
func (c *TaskServiceHTTPClientImpl) CompleteTask(ctx context.Context, in *CompleteTaskRequest, opts ...http.CallOption) (*CompleteTaskReply, error) {
	var out CompleteTaskReply
	pattern := "/api/v1/tasks/{id}/complete"
	path := binding.EncodeURL(pattern, in, false)
	opts = append([]http.CallOption{
		http.Operation(OperationTaskServiceCompleteTask),
		http.PathTemplate(pattern),
	}, opts...)
	err := c.cc.Invoke(ctx, "POST", path, in, &out, opts...)
	if err != nil {
		return nil, err
	}
	return &out, nil
}

// CreateTask CreateTask creates a new task.
func (c *TaskServiceHTTPClientImpl) CreateTask(ctx context.Context, in *CreateTaskRequest, opts ...http.CallOption) (*CreateTaskReply, error) {
	var out CreateTaskReply
	pattern := "/api/v1/tasks"
	path := binding.EncodeURL(pattern, in, false)
	opts = append([]http.CallOption{
		http.Operation(OperationTaskServiceCreateTask),
		http.PathTemplate(pattern),
	}, opts...)
	err := c.cc.Invoke(ctx, "POST", path, in, &out, opts...)
	if err != nil {
		return nil, err
	}
	return &out, nil
}

// DeleteTask DeleteTask removes a task.
func (c *TaskServiceHTTPClientImpl) DeleteTask(ctx context.Context, in *DeleteTaskRequest, opts ...http.CallOption) (*DeleteTaskReply, error) {
	var out DeleteTaskReply
	pattern := "/api/v1/tasks/{id}"
	path := binding.EncodeURL(pattern, in, true)
	opts = append([]http.CallOption{
		http.Operation(OperationTaskServiceDeleteTask),
		http.PathTemplate(pattern),
	}, opts...)
	err := c.cc.Invoke(ctx, "DELETE", path, nil, &out, opts...)
	if err != nil {
		return nil, err
	}
	return &out, nil
}

// GetTask GetTask returns one task by id.
func (c *TaskServiceHTTPClientImpl) GetTask(ctx context.Context, in *GetTaskRequest, opts ...http.CallOption) (*GetTaskReply, error) {
	var out GetTaskReply
	pattern := "/api/v1/tasks/{id}"
	path := binding.EncodeURL(pattern, in, true)
	opts = append([]http.CallOption{
		http.Operation(OperationTaskServiceGetTask),
		http.PathTemplate(pattern),
	}, opts...)
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
	}
	return &out, nil
}

// ListTasks ListTasks lists tasks for the current user with optional filters.
func (c *TaskServiceHTTPClientImpl) ListTasks(ctx context.Context, in *ListTasksRequest, opts ...http.CallOption) (*ListTasksReply, error) {
	var out ListTasksReply
	pattern := "/api/v1/tasks"
	path := binding.EncodeURL(pattern, in, true)
	opts = append([]http.CallOption{
		http.Operation(OperationTaskServiceListTasks),
		http.PathTemplate(pattern),
	}, opts...)
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
	}
	return &out, nil
}

// SkipTask SkipTask skips a task (todos are postponed to tomorrow).
func (c *TaskServiceHTTPClientImpl) SkipTask(ctx context.Context, in *SkipTaskRequest, opts ...http.CallOption) (*SkipTaskReply, error) {
	var out SkipTaskReply
	pattern := "/api/v1/tasks/{id}/skip"
	path := binding.EncodeURL(pattern, in, false)
	opts = append([]http.CallOption{
		http.Operation(OperationTaskServiceSkipTask),
		http.PathTemplate(pattern),
	}, opts...)
	err := c.cc.Invoke(ctx, "POST", path, in, &out, opts...)
	if err != nil {
		return nil, err
	}
	return &out, nil
}

// UpdateTask UpdateTask updates an existing task (title, difficulty, tags, skip, etc.).
func (c *TaskServiceHTTPClientImpl) UpdateTask(ctx context.Context, in *UpdateTaskRequest, opts ...http.CallOption) (*UpdateTaskReply, error) {
	var out UpdateTaskReply
	pattern := "/api/v1/tasks/{id}"
	path := binding.EncodeURL(pattern, in, false)
	opts = append([]http.CallOption{
		http.Operation(OperationTaskServiceUpdateTask),
		http.PathTemplate(pattern),
	}, opts...)
	err := c.cc.Invoke(ctx, "PUT", path, in, &out, opts...)
	if err != nil {
		return nil, err
	}
	return &out, nil
}
